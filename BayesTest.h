...
public:
	...
	explicit BayesTest(Forest& aForest, unsigned int aVerbose = 0,
			...
			mForest(aForest), mNumSites(mForest.getNumSites()), mSiteClassProb(
					BEB_DIMS * mNumSites), mVerbose(aVerbose), mPriors(
					mNumSites * BEB_NUM_CAT), mDependencies(mForest, aVerbose), mBEBset(
					mForest.getNumBranches()) {
	...
	///
	void extractPositiveSelSites(std::vector<unsigned int>& aPositiveSelSites,
...
private:
	...
	const static unsigned int BEB_NUM_CAT = BEB_N1D + 1 + BEB_N1D * BEB_N1D
			+ BEB_N1D;///< Total number of categories for w0 and w2 (it is com.ncatG in codeml.c)
...
public:
	...
	/// @param[in] aScales The two scales ([0] bg; [1] fg) to rescale the branch lengths. They are computed in H1.
	///
	void computeBEB(const std::vector<double>& aVars,
...
private:
	...
	std::vector<double> mPriors;///< Computed priors (each points to a list, one for each site)
	TreeAndSetsDependencies mDependencies;///< Dependency list for likelihood computation
	mfgProbabilityMatrixSetBEB mBEBset;	///< Probability matrix set to be used for likelihood computation
};
