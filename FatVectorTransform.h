...
public:
	...
	void setNodeExists(unsigned int aBranch, unsigned int aSite) {
		mNodeStatus[aBranch * mNumSites + aSite] =
...
private:
	...
	std::vector<std::vector<unsigned int> > mBranchByLevel;	///< Each level contains a list of branch numbers at this level. List start from the leaves.
