...
double MfgBayesTest::getGridParams(const std::vector<double>& aVars, const std::vector<double>& aSiteMultiplicity, std::set<int> aFgBranchSet, const std::vector<double>& aScales)
{
	// Parameters for w0, w1, w2 prior computation
	double prior_params[BEB_DIMS][BEB_N1D];
...
void MfgBayesTest::getIndexTernary(double* aProbX, double* aProbY, unsigned int aTriangleIdx)
{
	unsigned int ix = static_cast<unsigned int>(sqrt(static_cast<double>(aTriangleIdx)));
	unsigned int iy = aTriangleIdx - ix*ix;
...
void MfgBayesTest::computeBEB(const std::vector<double>& aVars, std::set<int> aFgBranchSet, const std::vector<double>& aScales)
{
	...
	// Prepare the priors list
	double scale1 = getGridParams(aVars, site_multiplicity, aFgBranchSet, aScales);
...
void MfgBayesTest::extractPositiveSelSites(std::vector<unsigned int>& aPositiveSelSites, std::vector<double>& aPositiveSelSitesProb) const
{
	// Prepare the output vectors
	aPositiveSelSites.clear();
	aPositiveSelSitesProb.clear();
