...
double BayesTest::getGridParams(const std::vector<double>& aVars,
		const std::vector<double>& aSiteMultiplicity, size_t aFgBranch,
...
#pragma omp section
		...
		for (size_t site = 0; site < mNumSites; ++site) {
			...
			mPriors[iw * mNumSites + site] =
					(p > 0) ? log(p) : -184.2068074395237; // If p < 0 then the value in codeml.c is: log(1e-80);
		}
	}
	...
	for (size_t site = 0; site < mNumSites; ++site) {
		...
		for (size_t k = 0; k < BEB_NUM_CAT; ++k) {
			mPriors[k * mNumSites + site] = exp(
					mPriors[k * mNumSites + site] - fh);
		}
...
void BayesTest::getIndexTernary(double* aProbX, double* aProbY,
	...
	unsigned int ix = static_cast<unsigned int>(sqrt(
			static_cast<double>(aTriangleIdx)));
...
void BayesTest::computeBEB(const std::vector<double>& aVars, size_t aFgBranch,
	...
	const std::vector<double>& site_multiplicity =
			mForest.getSiteMultiplicity();
	...
	for (unsigned int igrid = 0; igrid < BEB_NGRID; ++igrid) {
		...
		for (unsigned int codon_class = 0; codon_class < BEB_DIMS;
				++codon_class) {
	...
	// Print
	if (mVerbose > VERBOSE_ONLY_RESULTS) {
		std::cout << std::endl << "Posterior on the grid" << std::endl
				<< std::endl;
		...
		std::cout << std::endl << "Posterior for p0-p1 (see the ternary graph)"
				<< std::endl << std::endl;
		...
		std::cout << std::endl << "Sum of density on p0-p1 = "
				<< std::setprecision(4) << sum_postp0p1 << std::endl
				<< std::endl;
	}
	...
	if (mVerbose > VERBOSE_ONLY_RESULTS)
		std::cout << "log(fX) = "
				<< (fX + scale1 - BEB_DIMS * log(BEB_N1D * 1.)) << "	 Scales = "
				<< scale1 << " " << scale2 << std::endl;
	...
	for (unsigned int site = 0; site < mNumSites; ++site) {
		...
		for (unsigned int igrid = 0; igrid < BEB_NGRID; ++igrid) {
			...
			{
				fhk[codon_class] = codon_class_proportion[igrid * BEB_DIMS
						+ codon_class]
						* mPriors[iw[igrid * BEB_DIMS + codon_class] * mNumSites
								+ site];
				fh += fhk[codon_class];
			}
			...
			for (codon_class = 0; codon_class < BEB_DIMS; ++codon_class) {
				...
				mSiteClassProb[codon_class * mNumSites + site] += exp(
						t - scale1);
			}
		}
		for (unsigned int j = 0; j < BEB_DIMS; ++j)
...
void BayesTest::printPositiveSelSites(size_t aFgBranch) const {
	...
	for (unsigned int site = 0; site < mNumSites; ++site) {
		...
		if (prob > MIN_PROB) {
			...
			if (print_title) {
				std::cout << std::endl
						<< "Positive selection sites and their probabilities for foreground branch "
						<< aFgBranch << std::endl;
				print_title = false;
			}
			...
			std::pair<std::multimap<size_t, size_t>::const_iterator,
					std::multimap<size_t, size_t>::const_iterator> ret(
					internal_to_original_site.equal_range(site));
			std::multimap<size_t, size_t>::const_iterator it(ret.first);
			const std::multimap<size_t, size_t>::const_iterator end(ret.second);
			for (; it != end; ++it) {
				ordered_map.insert(
						std::pair<size_t, size_t>(it->second, current_idx));
...
/*                                     the Bayes Test Class Routines (with multiple foregrounds)                            */

double MfgBayesTest::getGridParams(const std::vector<double>& aVars,
		const std::vector<double>& aSiteMultiplicity,
		std::set<int> aFgBranchSet, const std::vector<double>& aScales) {
	// Parameters for w0, w1, w2 prior computation
	double prior_params[BEB_DIMS][BEB_N1D];
...
void MfgBayesTest::computeBEB(const std::vector<double>& aVars,
	...
	// Prepare the priors list
	double scale1 = getGridParams(aVars, site_multiplicity, aFgBranchSet,
			aScales);
...
void MfgBayesTest::printPositiveSelSites(std::set<int> aFgBranchSet) const {
	...
	for (unsigned int site = 0; site < mNumSites; ++site) {
		...
		if (prob > MIN_PROB) {
			...
			if (print_title) {
				std::cout << std::endl
						<< "Positive selection sites and their probabilities for foreground branch(es) ";
