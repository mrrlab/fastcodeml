...
void WriteResults::outputResults(void) {
	...
	for (size_t branch = min_branch; branch <= max_branch; ++branch) {
		out << "Branch: " << std::setw(4) << branch << std::endl << std::endl
				<< "	LnL0: ";
		...
		if (im == mLnL[0].end()) {
			...
			out << std::setw(22) << std::setprecision(15) << std::fixed
					<< im->second;

			ims = mParamStr[0].find(branch);
			// This output does not work on BG/Q.
			out << std::endl << std::fixed << "Branch lengths:" << ims->second
					<< std::endl;
		}
		out << "  LnL1: ";
	...
	for (size_t branch = min_branch; branch <= max_branch; ++branch) {
		std::map<size_t,
				std::pair<std::vector<unsigned int>, std::vector<double> > >::const_iterator ipss;
		...
		if (ipss != mPositiveSelSites.end()) {
			...
			for (size_t s = 0; s < ns; ++s) {
				size_t i = idx[s];
				out << "PositiveSelectionSite for branch: " << std::setw(4)
						<< branch;
				out << "  Site: " << std::setw(6) << site[i] + 1 << "  Prob: "
...
const std::vector<size_t>& WriteResults::orderSites(
...
void WriteResults::savePositiveSelSites(size_t aFgBranch,
	...
	mPositiveSelSites[aFgBranch] = std::make_pair(aPositiveSelSites,
