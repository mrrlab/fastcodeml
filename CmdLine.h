...
public:
	...
	///
	CmdLine() :
			mDeltaValueForGradient(0.0), mRelativeError(1e-3), mTreeFile(NULL), mGeneFile(
					NULL), mGraphFile(NULL), mResultsFile(NULL), mVerboseLevel(
					VERBOSE_ONLY_RESULTS), mSeed(0), mBranchAll(false),
			//mBranchStart(UINT_MAX),
			//mBranchEnd(UINT_MAX),
			mExportComputedTimes(UINT_MAX), mComputeHypothesis(UINT_MAX), mOptimizationAlgo(
					22), mExtraDebug(0), mMaxIterations(MAX_ITERATIONS), mIgnoreFreq(
					false), mDoNotReduceForest(false), mBranchLengthsFromFile(
					false), mNoMaximization(false), mTrace(false),
#ifdef _OPENMP
					mNumThreads(omp_get_max_threads()),
#else
					mNumThreads(1),
#endif
					mForceSerial(false), mBranchFromFile(false), mInitH0fromH1(
							false), mInitFromParams(false), mCleanData(false), mStopIfNotLRT(
							true),
//		mCmdLineImpl(NULL),
