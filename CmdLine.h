...
public:
  ...
  CmdLine()
      : mDeltaValueForGradient(0.0), mRelativeError(1e-3), mTreeFile(NULL),
        ...
        mNumThreads(1),
#endif
        mForceSerial(false), mBranchFromFile(false), mInitH0fromH1(false),
        mInitFromParams(false), mCleanData(false), mStopIfNotLRT(true),
#ifdef USE_AGGREGATION
        mAggregate(0),
#endif

        //		mCmdLineImpl(NULL),
        mFixedBranchLength(false), mCmdLineImpl(NULL)
...
public:
  double mDeltaValueForGradient; ///< The variable increment to compute gradient
                                 ///(zero means use a hardcoded default value)
  ...
  const char *mGraphFile; ///< If not null export the forest to this file in GML
  /// format to be visualized using R igraph package or
  /// yEd editor
  const char *mResultsFile;   ///< File to which the results should be written
  unsigned int mVerboseLevel; ///< Verbosity level. 0: no messages; 1: basic
  ...
  unsigned int mComputeHypothesis; ///< If set to 0 compute only H0, if set to 1
  /// compute only H1, otherwise compute both
...
#endif
  bool mIgnoreFreq; ///< Ignore the computed codon frequencies and set them all
  /// to 1/61
  bool mDoNotReduceForest; ///< If true do not reduce the forest merging common
  /// subtrees
  bool mBranchLengthsFromFile; ///< The initial value of the branch lengths is
  /// taken from the phylo tree file
  bool mNoMaximization; ///< Only the first step of the likelihood maximization
  /// is taken
  bool mTrace; ///< Trace the optimization steps
  unsigned int
      mNumThreads; ///< Number of threads (if 1 the parallelization is disabled)
  bool mForceSerial;    ///< Disable all parallelism
  bool mBranchFromFile; ///< Read the foreground branch to use from the phylo
  /// tree file (it is marked as #1)
  bool mInitH0fromH1; ///< If set starts the H0 computation from the H1 results
  bool mInitFromParams; ///< Initialize times from phylo tree and the other from
  /// values hardcoded or entered on the command line
...
private:
  struct CmdLineImpl;
  CmdLineImpl *mCmdLineImpl; ///< Implementation so this structure could be used
  /// without other modules be aware of its internals
};
