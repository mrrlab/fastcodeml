...
CmdLine::~CmdLine() { delete mCmdLineImpl; }
...
void CmdLine::parseCmdLine(int aCnt, char **aVal) {
  ...
  // Setup the command line parser. First the identifiers for the various
  // options
  enum {
    OPT_VERBOSE,
    OPT_QUIET,
...
#ifdef USE_AGGREGATION
      {OPT_AGGREGATE, "-a", SO_REQ_SEP, "Aggregation (0: off, 1: full, 2: "
                                        "without positions having distance of "
                                        "three between codons, default:0)"},
  ...
  // Declare our options parser, pass in the arguments from main as well as our
  ...
  while (args.Next()) {
    ...
    case OPT_AGGREGATE:
      tmpi = atoi(args.OptionArg());
      if (tmpi < 0 || tmpi > 2)
        throw FastCodeMLFatal("Invalid aggregation mode");
      mAggregate = static_cast<unsigned int>(tmpi);
      // Aggregation doesn't work with do not reduce
      mDoNotReduceForest = true;
      break;
#endif
    }
