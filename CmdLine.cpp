...
void CmdLine::parseCmdLine(int aCnt, char **aVal) {
	...
	enum {
		OPT_VERBOSE, OPT_QUIET, OPT_HELP, OPT_SEED, OPT_BRANCH_ALL,
		//OPT_BRANCH_START,
		//OPT_BRANCH_END,
		OPT_IGNORE_FREQ,
		OPT_EXPORT,
...
//		OPT_FORCE_SERIAL,
	...
	CSimpleOpt::SOption parser_options[] =
			{
					...
					{ OPT_VERBOSE, "--verbose", SO_REQ_SEP, "" }, { OPT_QUIET,
							"-q", SO_NONE, "No messages except results" }, {
							OPT_QUIET, "--quiet", SO_NONE, "" },
					{ OPT_EXPORT, "-e", SO_REQ_SEP,
							"Export forest in GML format (if %03d or @03d is present, one is created for each fg branch)" },
					{ OPT_EXPORT, "--export", SO_REQ_SEP, "" }, { OPT_TRACE,
							"-r", SO_NONE, "Trace the maximization run" }, {
					...
					{ OPT_ONE_HYP_ONLY, "-hy", SO_REQ_SEP,
					...
					{ OPT_INIT_PARAM, "-p", SO_REQ_SEP,
							"Pass initialization parameter in the form: P=value (P: w0, k, p0, p1, w2) -> e.g. -p w0=1.1 -p p0=2.1 ..." },
	...
	while (args.Next()) {
		...
		if (args.LastError() == SO_OPT_INVALID) {
			std::ostringstream o;
			o << "Error: " << mCmdLineImpl->getLastErrorText(args) << ": "
					<< args.OptionText() << std::endl;
			throw FastCodeMLFatal(o);
		}
		if (args.LastError() != SO_SUCCESS) {
			std::ostringstream o;
			o << "Error: " << mCmdLineImpl->getLastErrorText(args) << " for: "
					<< args.OptionText() << std::endl;
			throw FastCodeMLFatal(o);
		}
		...
		case OPT_VERBOSE:
			if (mVerboseLevel == VERBOSE_NONE)
				break; // Ignore option if quiet is set
			tmpi = atoi(args.OptionArg());
			if (tmpi < 0)
				throw FastCodeMLFatal("Invalid verbose level");
		...
		case OPT_SEED:
			tmpi = atoi(args.OptionArg());
			if (tmpi < 0)
				throw FastCodeMLFatal("Invalid seed value");
		...
		case OPT_COMP_TIMES:
			mExportComputedTimes = static_cast<unsigned int>(atoi(
					args.OptionArg()));
			if (mExportComputedTimes > 1)
		...
		case OPT_ONE_HYP_ONLY:
			mComputeHypothesis = static_cast<unsigned int>(atoi(
					args.OptionArg()));
			if (mComputeHypothesis != 0 && mComputeHypothesis != 1)
				throw FastCodeMLFatal("Invalid hypothesis specified");
			break;
		...
		case OPT_OPTIM_ALGO:
			mOptimizationAlgo =
					static_cast<unsigned int>(atoi(args.OptionArg()));
			break;

		case OPT_DELTA_VAL:
			mDeltaValueForGradient = atof(args.OptionArg());
			if (mDeltaValueForGradient < 0.0)
		...
		case OPT_REL_ERROR:
			...
			if (mRelativeError <= 0.0)
				throw FastCodeMLFatal("Invalid relative error value");
			break;
		...
		case OPT_MAX_ITER:
			...
			if (tmpi <= 0)
				throw FastCodeMLFatal(
						"Invalid max number of iterations. Should be > 0");
	...
	case 1:
		std::cout << "Missing PHYLIP CODON ALIGNMENT file" << std::endl
				<< std::endl;
		std::cout << std::endl << "------------------" << std::endl
				<< "FastCodeML V" << version << std::endl
				<< "------------------" << std::endl;
	...
	if (!mGraphFile)
		mExportComputedTimes = UINT_MAX;
	if (mComputeHypothesis < 2)
		mInitH0fromH1 = false;
	if (mComputeHypothesis == 0 && mExportComputedTimes < 2)
