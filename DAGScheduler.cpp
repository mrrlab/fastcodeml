...
void DAGScheduler::loadDependency(unsigned int aCopyId, const void *aDependsOn,
  ...
  // Insert the pair in the reference counter list (increment if present the
  // dependant; do nothing if the dependent is already here)
  mRefCounter.insert(std::pair<const void *, int>(aDependsOn, 0));
  std::pair<std::map<const void *, int>::iterator, bool> ret =
      mRefCounter.insert(std::pair<const void *, int>(aDependant, 1));
  if (ret.second == false)
    ++(ret.first->second);
}
...
void *DAGScheduler::getNext(void) { return 0; }
...
void DAGScheduler::dumpDAG(std::ostream &aOut) const {
  ...
  for (; irc != mRefCounter.end(); ++irc) {
    ...
    int cnt = irc->second;
    std::cout << std::hex << k << ' ' << std::dec << std::setw(6) << cnt
              << std::endl;
  }
}
