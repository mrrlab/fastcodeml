...
public:
	...
	///
	const double* getInvCodonFrequencies(void) const {
		return &mCodonFreqInv[0];
	}
	...
	///
	unsigned int getNumGoodCodons(void) const {
		return mNumGoodCodons;
	}
	...
	///
	const double* getCodonFreqInv2(void) const {
		return &mCodonFreqInv2[0];
	}
...
protected:
	...
	///
	CodonFrequencies() :
			mCodonFrequencies(N, 1. / static_cast<double>(N)), mCodonFreqSqrt(N,
					1. / sqrt(static_cast<double>(N))), mCodonFreqInv(N,
					static_cast<double>(N)), mCodonFreqInv2(N,
					static_cast<double>(N * N)), mNumGoodCodons(N) {
		mGoodCodon.set();
	}
};
