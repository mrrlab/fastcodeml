...
void CodonFrequencies::setCodonFrequencies(
	...
	if (aModel == CODON_FREQ_MODEL_F3X4) {
		...
		if (no_ambiguous_codons) {
			...
			for (size_t i = 0; i < cnt; ++i)
				codon_count[aCodons[i][1]] +=
						static_cast<double>(aCodons[i][0]);
			...
			for (int k = 0; k < CODON_FREQ_MAX_ITER; ++k) {
				...
				if (aShowMessages)
					std::cout << "  mCodonFrequencies change after iteration "
							<< k << " = " << std::scientific << err
							<< std::fixed << std::endl << std::endl;
...
void CodonFrequencies::setCodonFrequenciesF3x4(
	...
	for (k = 0; k < N64; ++k) {
		...
		mCodonFrequencies[kk] = fb3x4sg[k / 16] * fb3x4sg[4 + (k / 4) % 4]
				* fb3x4sg[8 + k % 4];
	}
	double t = 0;
	for (k = 0; k < N; ++k)
	...
	if (aShowMessages) {
		for (k = 0; k < N64; ++k) {
			...
			std::cout << std::fixed << std::setprecision(8) << std::setw(12)
					<< v;
			if (k % 4 == 3)
				std::cout << std::endl;
		}
