...
void Newick::readFile(const char *aFilename) {
	...
	if (p1 == std::string::npos) {
		std::ostringstream o;
		o << "File \"" << aFilename
				<< "\" is empty (cannot find starting parenthesis)."
				<< std::endl;
		throw FastCodeMLFatal(o);
	}
	...
	if (p2 == std::string::npos) {
		std::ostringstream o;
		o << "File \"" << aFilename
				<< "\" is empty (cannot find ending semicolon)." << std::endl;
		throw FastCodeMLFatal(o);
	}
...
void Newick::loadTreeFromString(const std::string& aTreeAsString) {
	...
	if (info.full) {
		...
		if (mVerboseLevel >= VERBOSE_MORE_DEBUG) {
			...
			const std::vector<TreeNode *>::const_iterator end(
					mLeavesSpecies.end());
			for (; isp != end; ++isp)
...
int Newick::printTreeAnnotated(std::ostream& aOut, TreeNode *aNode, int aBranch,
	...
	if (!aNode) {
		if (wLeaves)
			aOut << "Annotated Newick Tree (*N marks the branch N)"
					<< std::endl;
		else
...
int Newick::printTreeAnnotatedWithEstLens(std::ostream& aOut, TreeNode *aNode,
		int aBranch, bool wLeaves, std::vector<double>* mVar) const {
	TreeNode *m;
	unsigned int idx;
	int branch_idx = aBranch;
	...
	if (!aNode) {
		...
		for (idx = 0; (m = mTreeRoot.getChild(idx)) != NULL; ++idx) {
			if (idx > 0)
				aOut << ',';
			branch_idx = printTreeAnnotatedWithEstLens(aOut, m, branch_idx,
					wLeaves, mVar);
		}
		aOut << ')';
