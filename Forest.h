...
public:
	...
	explicit Forest(unsigned int aVerbose = 0) :
			mNumSites(0), mCodonFreq(NULL), mInvCodonFreq(NULL), mInv2CodonFreq(
					NULL), mNumBranches(0), mNumInternalBranches(0), mMarkedInternalBranch(
					UINT_MAX), mVerbose(aVerbose) {
	}
	...
	///
	void loadTreeAndGenes(const PhyloTree& aTree, const Genes& aGenes,
	...
	///
	size_t getNumInternalBranches(void) const {
		return mNumInternalBranches;
	}
	...
	///
	std::set<int> getMarkedBranches(void) const {
		return mMarkedBranches;
	}

	/// Get all internal branches
	///
	/// @return The internal branch index of the internal branches in the tree file. UINT_MAX otherwise.
	///
	std::set<int> getInternalBranches(void) const {
		return mInternalBranches;
	}
	...
	///
	const std::vector<std::vector<unsigned int> >& getTreeDependencies(
			void) const {
	...
	///
	const std::vector<std::vector<unsigned int> >& getTreeRevDependencies(
			void) const {
...
private:
	...
	std::vector<unsigned int> mTableInternalToBranchID;	///< Map from internal branch number to branch number
	...
	std::set<int> mInternalBranches;///< id of the internal branches in the tree file
