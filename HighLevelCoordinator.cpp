...
enum JobType {
  ...
  JOB_BEB = 2,     ///< BEB computation (done only if H0 and H1 already done and
                   ///passing the likelihood ratio test)
...
struct HighLevelCoordinator::WorkTable {
  ...
  struct ResultSet {
    ...
    std::vector<int>
        mPositiveSelSites; ///< Sites (if any) under positive selection
    std::vector<double> mPositiveSelProbs; ///< Corresponding probabilities

    /// Default constructor.
    ///
    ResultSet() { mLnl[0] = mLnl[1] = -DBL_MAX; }
  };

  size_t mNumInternalBranches;     ///< Number of internal branches that can be
                                   ///marked as foreground branch.
  ...
  /// Constructor
  ///
  /// @param[in] aNumInternalBranches Number of internal branches that can be
  /// marked as foreground branch.
  ///
  ...
  /// Get the next job to be executed. If no more jobs then set aJob to shutdown
  /// and return false
  ///
  /// @param[out] aJob The job request: [0] is set to the kind of job (JOB_H0,
  /// JOB_H1, JOB_BEB, JOB_SHUTDOWN);
  ///									  [1] to the fg branch number (or zero for
  ///JOB_SHUTDOWN);
  ///									  [2] zero or the number of variables for a
  ...
  ///
  /// @return The identifier of the finished job (it is
  /// branch*JOBS_PER_BRANCH+job_type)
  ///
  ...
  ///
  void printVariables(size_t aBranch, unsigned int aHyp,
  ...
  ///
  /// @param[in] aHypothesisToDo The hypothesis that should be computed. If
  /// different from 0 or 1 do nothing.
  ///
...
bool HighLevelCoordinator::WorkTable::getNextJob(int *aJob, int aRank,
  ...
  for (size_t branch = 0; branch < mNumInternalBranches; ++branch) {
    ...
    if (mJobStatus[idx] == JOB_WAITING &&
        mJobStatus[branch * JOBS_PER_BRANCH + JOB_H1] == JOB_COMPLETED) {
      aJob[0] = JOB_H0;
      aJob[1] = static_cast<int>(branch);
      aJob[2] =
          (aInitFromH1)
              ? static_cast<int>(mResults[branch].mHxVariables[1].size()) + 1
              : 1; // Send the lnl of the corresponding H1 step or
                   // mResults[branch].mHxVariables[1]
      mJobStatus[idx] = JOB_ASSIGNED;
      mWorkList[idx] = aRank;
      return true;
    }
  ...
  for (size_t branch = 0; branch < mNumInternalBranches; ++branch) {
    ...
    if (mJobStatus[branch * JOBS_PER_BRANCH + JOB_BEB] != JOB_WAITING)
      continue;

    // The corresponding H0 and H1 jobs should be completed
    if (mJobStatus[branch * JOBS_PER_BRANCH + JOB_H0] != JOB_COMPLETED ||
        mJobStatus[branch * JOBS_PER_BRANCH + JOB_H1] != JOB_COMPLETED)
      continue;

    // If the previous results do not pass the LRT or H0 has been interrupted,
    // then skip the BEB computation
    if (mResults[branch].mLnl[0] == DBL_MAX ||
        !BranchSiteModel::performLRT(mResults[branch].mLnl[0],
                                     mResults[branch].mLnl[1])) {
...
void HighLevelCoordinator::WorkTable::printFinishedBranch(int aIdx) const {
  ...
  if (mJobStatus[i + JOB_H0] == JOB_COMPLETED &&
      mJobStatus[i + JOB_H1] == JOB_COMPLETED) {
    // If the previous results do not pass the LRT or H0 has been interrupted,
    // then skip the BEB computation
    if (mJobStatus[i + JOB_BEB] == JOB_COMPLETED ||
        mResults[branch].mLnl[0] == DBL_MAX ||
        !BranchSiteModel::performLRT(mResults[branch].mLnl[0],
                                     mResults[branch].mLnl[1])) {
      std::cout << "Branch " << std::setw(3) << branch << " completed"
                << std::endl;
    }
  }
}
...
void HighLevelCoordinator::WorkTable::checkAllJobsDone(void) const {
  ...
  for (size_t i = 0; i < mJobStatus.size(); ++i) {
    if (mJobStatus[i] != JOB_COMPLETED && mJobStatus[i] != JOB_SKIP) {
      std::cout << "Job kind: " << (i % JOBS_PER_BRANCH) << " for branch "
                << (i / JOBS_PER_BRANCH)
                << " still in status: " << mJobStatus[i] << std::endl;
      any_error = true;
    }
  }
...
void HighLevelCoordinator::WorkTable::skipOutsideRange(size_t aBranchStart,
                                                       size_t aBranchEnd) {
  for (size_t branch = 0; branch < mNumInternalBranches; ++branch) {
    // Don't touch jobs in the range
    if (branch >= aBranchStart && branch <= aBranchEnd)
      continue;
...
void HighLevelCoordinator::WorkTable::skipOtherHypothesis(
    unsigned int aHypothesisToDo) {
...
void HighLevelCoordinator::WorkTable::printVariables(size_t aBranch,
                                                     unsigned int aHyp,
                                                     std::ostream &aOut) const {
  aOut << "Optimized variables for H" << aHyp << " for fg branch " << aBranch
       << std::endl;
  ...
  int num_times =
      static_cast<int>(mResults[aBranch].mHxVariables[aHyp].size()) -
      ((aHyp) ? 7 : 4); // for H1 is 5 + 2 scale factors
  double v0 = 0;
  std::vector<double>::const_iterator ix(
      mResults[aBranch].mHxVariables[aHyp].begin());
...
HighLevelCoordinator::HighLevelCoordinator(int *aRgc, char ***aRgv)
    : mVerbose(0), mRank(INVALID_RANK), mSize(0), mNumInternalBranches(0),
      mWorkTable(NULL) {
#ifdef _OPENMP
#ifdef VTRACE
  const int requested = MPI_THREAD_SINGLE;
#else
  const int requested = (omp_get_max_threads() <= 1)
                            ? MPI_THREAD_SINGLE
                            : MPI_THREAD_FUNNELED; // Change to
                                                   // MPI_THREAD_SERIALIZED if
                                                   // master process do more
#endif
#else
  const int requested = MPI_THREAD_SINGLE;
#endif
...
bool HighLevelCoordinator::startWork(Forest &aForest, const CmdLine &aCmdLine) {
  // You need more than 2 MPI process to take advantage of it. Otherwise run as
  // single process, OpenMP only.
  if (mSize < 3)
    return false;
  ...
  if (mRank == MASTER_JOB) {
    ...
    if (mVerbose >= VERBOSE_INFO_OUTPUT) {
      ...
      if (surplus > 0)
        std::cout << "Too many MPI jobs: " << surplus
                  << " of them will not be used." << std::endl;
      else if (surplus < 0)
        std::cout << "For top performances " << -surplus
                  << " more MPI jobs needed." << std::endl;
    }
    ...
    if (!do_all)
      mWorkTable->skipOutsideRange(branch_start, branch_end);

    // If only one hypothesis requested skip the other (do nothing if both
    // requested)
...
void HighLevelCoordinator::doMaster(WriteResults &aOutputResults,
  ...
  for (;;) {
    ...
    int job_request[2];
    MPI_Status status;
    MPI_Recv(static_cast<void *>(job_request), 2, MPI_INTEGER, MPI_ANY_SOURCE,
             MSG_WORK_REQUEST, MPI_COMM_WORLD, &status);
    int worker = status.MPI_SOURCE;

    // Act on the request (job_request[0] values are from the JobRequestType
    // enum, [1] is the response length)
    ...
    case REQ_HX_RESULT: {
      ...
      MPI_Recv(static_cast<void *>(&results_double[0]), job_request[1],
               MPI_DOUBLE, worker, MSG_GET_RESULTS, MPI_COMM_WORLD, &status);
      ...
      // For H1 there are also the two scale values at the end of variables for
      // BEB computation
      double lnl = results_double[job_request[1] - 1];
      mWorkTable->mResults[branch].mLnl[h] = lnl;
      mWorkTable->mResults[branch].mHxVariables[h].assign(
          results_double.begin(), results_double.end() - 1);
      ...
      if (mVerbose >= VERBOSE_INFO_OUTPUT)
        mWorkTable->printFinishedBranch(idx);

      // Output a status message
      if (mVerbose >= VERBOSE_MPI_TRACE) {
        if (lnl < DBL_MAX)
          std::cout << std::fixed << std::setprecision(8) << "Lnl: " << lnl
                    << " for H" << h << " from worker " << worker << std::endl;
        else
          std::cout << std::fixed << "Lnl: NA for H" << h << " from worker "
                    << worker << std::endl;
      }
    } break;
    ...
    case REQ_BEB_RESULT: {
      ...
      if (job_request[1] > 0) {
        ...
        MPI_Recv(static_cast<void *>(&results_integer[0]), job_request[1],
                 MPI_INTEGER, worker, MSG_GET_RESULTS, MPI_COMM_WORLD, &status);
      }
      ...
      // Save all results (positive selection sites and corresponding
      // probability)
      ...
      for (int i = 0; i < job_request[1] / 2; ++i) {
        ...
        double prob =
            static_cast<double>(results_integer[2 * i + 1]) / PROB_SCALING;
        mWorkTable->mResults[branch].mPositiveSelProbs.push_back(prob);
      }

      // If there are sites under positive selection, save them for the results
      // file
      if (job_request[1] > 0) {
        std::vector<unsigned int> sites;
        for (int i = 0; i < job_request[1] / 2; ++i) {
          unsigned int u =
              static_cast<unsigned int>(results_integer[2 * i + 0]);
          sites.push_back(u);
        }
        aOutputResults.savePositiveSelSites(
            static_cast<size_t>(branch), sites,
            mWorkTable->mResults[branch].mPositiveSelProbs);
      }
      ...
      // Output a status message
      if (mVerbose >= VERBOSE_MPI_TRACE)
        std::cout << "BEB num of results: " << job_request[1] / 2
                  << " from worker " << worker << std::endl;
    } break;
    ...
    // Send work packet or shutdown request (job[0] is the step to be done,
    // job[1] is the fg branch, job[2] the length of the additional data)
    int job[3];
    mWorkTable->getNextJob(job, worker, aCmdLine.mInitH0fromH1);
    MPI_Send(static_cast<void *>(job), 3, MPI_INTEGER, worker, MSG_NEW_JOB,
             MPI_COMM_WORLD);

    // For BEB send the variables from H1; for H0 send the lnl value from
    // corresponding H1
    if (job[2] > 0) {
      double *v;
      switch (job[0]) {
      case JOB_BEB:
        ...
        MPI_Send(static_cast<void *>(v), job[2], MPI_DOUBLE, worker,
                 MSG_NEW_JOB, MPI_COMM_WORLD);
        break;
      ...
      case JOB_H0:
        if (job[2] == 1) {
          ...
          results_double.assign(
              mWorkTable->mResults[job[1]].mHxVariables[1].begin(),
              mWorkTable->mResults[job[1]].mHxVariables[1].end());
    ...
    if (mVerbose >= VERBOSE_MPI_TRACE) {
      switch (job[0]) {
      case JOB_H0:
        std::cout << "Sent H0 [branch " << job[1] << "] to " << worker
                  << std::endl;
        break;
      case JOB_H1:
        std::cout << "Sent H1 [branch " << job[1] << "] to " << worker
                  << std::endl;
        break;
      case JOB_BEB:
        std::cout << "Sent BEB [branch " << job[1] << "] to " << worker
                  << std::endl;
        break;
      case JOB_SHUTDOWN:
        ...
        break;
      default:
        std::cout << "Sent " << job[0] << " [branch " << job[1] << "] to "
                  << worker << std::endl;
        break;
      }
    }
    ...
    if (job[0] == JOB_SHUTDOWN) {
      --num_workers;
      if (mVerbose >= VERBOSE_MPI_TRACE)
        std::cout << "Workers remaining: " << num_workers << std::endl;
      if (num_workers == 0)
        break;
    }
  }
  ...
  for (size_t branch = 0; branch < mNumInternalBranches; ++branch) {
    ...
    if (mWorkTable->mJobStatus[branch * JOBS_PER_BRANCH + JOB_H1] == JOB_SKIP)
      continue;
    ...
    if (mWorkTable->mResults[branch].mLnl[0] !=
            std::numeric_limits<double>::infinity() &&
        mWorkTable->mResults[branch].mLnl[0] != DBL_MAX)
      mWorkTable->printVariables(branch, 0, std::cout);
    if (mWorkTable->mResults[branch].mLnl[1] !=
        std::numeric_limits<double>::infinity())
  ...
  if (!branch_with_pos_selection.empty()) {
    ...
    for (; ib != end; ++ib) {
      ...
      for (size_t pss = 0; pss < branch_results.mPositiveSelSites.size();
           ++pss) {
        // Get probability
        double prob = branch_results.mPositiveSelProbs[pss];

        // Save site and probability to order output by site
        ordered_map.insert(std::pair<size_t, size_t>(
            branch_results.mPositiveSelSites[pss], current_idx));
        probs.push_back(prob);
        ++current_idx;
      }

      // Print site number and probability after mapping the site number to the
      // original value (and changing numbering so it starts from 1 and not
      // zero)
...
void HighLevelCoordinator::doWorker(Forest &aForest, const CmdLine &aCmdLine) {
  ...
  for (;;) {
    ...
    MPI_Request request;
    MPI_Isend(static_cast<void *>(job_request), 2, MPI_INTEGER, MASTER_JOB,
              MSG_WORK_REQUEST, MPI_COMM_WORLD, &request);

    // If needed (i.e. it is not a worker announcement), send step results
    switch (job_request[0]) {
    case REQ_HX_RESULT:
      MPI_Send(static_cast<void *>(&values_double[0]), job_request[1],
               MPI_DOUBLE, MASTER_JOB, MSG_GET_RESULTS, MPI_COMM_WORLD);
      break;

    case REQ_BEB_RESULT:
      if (job_request[1])
        MPI_Send(static_cast<void *>(&values_integer[0]), job_request[1],
                 MPI_INTEGER, MASTER_JOB, MSG_GET_RESULTS, MPI_COMM_WORLD);
      break;
    }

    // Receive the job to execute or the shutdown request (job[0] the request;
    // [1] fg branch; [2] optional number of variables)
    int job[3];
    MPI_Recv(static_cast<void *>(job), 3, MPI_INTEGER, MASTER_JOB, MSG_NEW_JOB,
             MPI_COMM_WORLD, &status);

    // If there is additional data
    if (job[2] > 0) {
      values_double.resize(static_cast<size_t>(job[2]));
      MPI_Recv(static_cast<void *>(&values_double[0]), job[2], MPI_DOUBLE,
               MASTER_JOB, MSG_NEW_JOB, MPI_COMM_WORLD, &status);
    }
    ...
    case JOB_H0: {
      ...
      if (aCmdLine.mInitH0fromH1 && job[2] > 1)
        h0.initFromResult(values_double,
                          static_cast<unsigned int>(values_double.size()) - 1u);
      else {
        if (aCmdLine.mInitFromParams)
          h0.initFromParams();
        if (aCmdLine.mBranchLengthsFromFile)
          h0.initFromTree();
      }

      // Get the lnl from the corresponding H1 step if any
      double threshold =
          (job[2] > 0) ? values_double.back() - THRESHOLD_FOR_LRT : 0.;

      // Compute H0
      double lnl = h0(static_cast<size_t>(job[1]),
                      aCmdLine.mStopIfNotLRT && job[2] > 0, threshold);
    ...
    case JOB_H1: {
      ...
      // Assemble the results to be passed to the master (variables, lnl and
      // scales for BEB)
    ...
    case JOB_BEB: {
      ...
      if (num_sites) {
        // Assemble consecutive pairs (site, probability) into an array of
        // integers
        values_integer.clear();
        for (size_t i = 0; i < num_sites; ++i) {
          values_integer.push_back(positive_sel_sites[i]);
          int v =
              static_cast<int>(positive_sel_sites_prob[i] * PROB_SCALING + 0.5);
