...
void Forest::loadTreeAndGenes(
    ...
    CodonFrequencies::CodonFrequencyModel aCodonFrequencyModel, int aAggregate)
#else
void Forest::loadTreeAndGenes(
    const PhyloTree &aTree, const Genes &aGenes,
...
#pragma omp task untied
#endif
      {
        ...
        if ((tmp_roots + site)->mObservedCodons.size() > 0)
          g = computeLikelihoodsWalkerAG(tmp_roots + site, aSet, set_idx,
                                         (tmp_roots + site)->mObservedCodons,
                                         (tmp_roots + site)->mMapCodonToState);
        else
        ...
        /*
        std::cout << "g=";
        for (std::vector<int>::const_iterator it=mObservedCodons[site].begin();
        it!=mObservedCodons[site].end(); ++it)
                std::cout << g[*it] << " ";
        std::cout << g[N+1] << "\n";
        */
        if ((tmp_roots + site)->mObservedCodons.size() > 0)
          likelihoods[set_idx * mNumSites + site] =
              dotn(mCodonFreq, g, mObservedCodons[site]) * g[N];
        else
...
#ifdef USE_AGGREGATION
double *Forest::computeLikelihoodsWalkerAG(
    const ForestNode *aNode, const ProbabilityMatrixSet &aSet,
    unsigned int aSetIdx, const std::vector<int> &aObservedCodons,
    const std::vector<int> &aMapCodonToState) {
  ...
  for (unsigned int idx = 0; idx < nc; ++idx) {
    ...
    if (aNode->isSameTree(idx)) {
      ...
      if (leaf_codon >= 0) {
        if (first) {
          aSet.doTransitionAtLeafAG(aSetIdx, branch_id, leaf_codon, anode_prob,
                                    aObservedCodons, aMapCodonToState);
          ...
          double *x = anode_other_tree_prob
                          ...
                          : temp;
          aSet.doTransitionAtLeafAG(aSetIdx, branch_id, leaf_codon, x,
                                    aObservedCodons, aMapCodonToState);
        ...
        if (first) {
          double *g = computeLikelihoodsWalkerAG(
              m, aSet, aSetIdx, aObservedCodons, aMapCodonToState);
          aSet.doTransitionAG(aSetIdx, branch_id, g, anode_prob,
                              aObservedCodons, aMapCodonToState);
          ...
          double *g = computeLikelihoodsWalkerAG(
              m, aSet, aSetIdx, aObservedCodons, aMapCodonToState);
          aSet.doTransitionAG(aSetIdx, branch_id, g, x, aObservedCodons,
                              aMapCodonToState);
