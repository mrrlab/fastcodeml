...
const unsigned char ForestNode::mMaskTable[MAX_NUM_CHILDREN] = { 0x1, 0x2, 0x4,
		0x8, 0x10, 0x20, 0x40, 0x80 };

void Forest::loadTreeAndGenes(const PhyloTree& aTree, const Genes& aGenes,
	...
	for (size_t site = 0; site < mNumSites; ++site) {
		...
		aTree.cloneTree(&mRoots[site], static_cast<unsigned int>(site),
				mNumSites, mProbs);
		...
		for (; il != end; ++il) {
			...
			(*il)->mPreprocessingSupport->mSubtreeCodonsSignature.push_back(
					codon);
...
void Forest::reduceSubtreesWalker(ForestNode* aNode,
	...
	for (i = 0; i < nc; ++i) {
		...
		if (aNode->mChildrenList[i]->mPreprocessingSupport->mSubtreeCodonsSignature
				== aNodeDependent->mChildrenList[i]->mPreprocessingSupport->mSubtreeCodonsSignature) {
...
std::ostream& operator<<(std::ostream& aOut, const Forest& aForest) {
	...
	aOut << "Reduced branches:	 " << std::fixed << std::setw(7) << cnt
			...
			<< static_cast<double>(cnt * 100.)
					/ static_cast<double>(aForest.mNumBranches
							* aForest.mNumSites) << '%' << std::endl;
	aOut << "Aggressive reduct.: " << std::fixed << std::setw(7)
			<< cntAggressive << std::setw(8) << std::setprecision(2)
...
void Forest::getEffortPerSite(std::vector<unsigned int>& aEfforts,
	...
	for (size_t i = 0; i < mNumSites; ++i) {
		unsigned int total_cost = mRoots[i].getCost(aCostAtLeaf, aCostIntern,
				aCostPtr);
...
void Forest::mapInternalToBranchIdWalker(const ForestNode* aNode,
	...
	for (unsigned int i = 0; i < nc; ++i) {
		...
		if (m->mInternalNodeId != UINT_MAX)
			aMapInternalToBranchID.insert(
					std::pair<unsigned int, unsigned int>(m->mInternalNodeId,
							m->mBranchId));
		...
		for (unsigned int i = 0; i < nc; ++i) {
			...
			if (aNode->isSameTree(i)) {
				...
				if (!other->mOtherTreeProb[i]) {
					double* pd =
							static_cast<double*>(alignedMalloc(
									VECTOR_SLOT * Nt * sizeof(double),
									CACHE_LINE_ALIGN));
					if (!pd)
						throw FastCodeMLMemoryError(
								"Cannot allocate mOtherTreeProb");
...
#pragma omp task untied
#endif
			{
				// Compute likelihood array at the root of one tree (the access order is the fastest)
				const unsigned int tmp = tmp_ivs[i];
				const unsigned int site = TreeAndSetsDependencies::getSiteNum(
						tmp);
				const unsigned int set_idx = TreeAndSetsDependencies::getSetNum(
						tmp);

				const double* g = computeLikelihoodsWalkerTC(tmp_roots + site,
						aSet, set_idx);
...
double* Forest::computeLikelihoodsWalkerTC(const ForestNode* aNode,
		const ProbabilityMatrixSet& aSet, unsigned int aSetIdx) {
	...
	for (unsigned int idx = 0; idx < nc; ++idx) {
		...
		if (aNode->isSameTree(idx)) {
			...
			if (leaf_codon >= 0) {
				if (first) {
					aSet.doTransitionAtLeaf(aSetIdx, branch_id, leaf_codon,
							anode_prob);
#ifdef USE_CPV_SCALING
					...
					if (anode_other_tree_prob)
						memcpy(anode_other_tree_prob + VECTOR_SLOT * aSetIdx,
								anode_prob, N * sizeof(double));
#endif
					first = false;
				} else {
#ifdef USE_CPV_SCALING
...
double* Forest::computeLikelihoodsWalkerTC(const ForestNode* aNode,
		const mfgProbabilityMatrixSet& aSet, unsigned int aSetIdx) {
	double* anode_prob = aNode->mProb[aSetIdx];
	const unsigned int nc = aNode->mChildrenCount;
