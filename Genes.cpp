...
#endif
{
  ...
  if (cnt == 0) {
    ...
    for (size_t i = 0; i < cnt; ++i)
      aLeaveProbVect[mCurrentPositions[i]] = prob;
    if (aAggregate > 0) {
      aLeaveProbVect[N + 1] = prob; // Set to 1. to have the CPV initialized to
                                    // all 1/cnt instead of 1

      aLeaveProbVect[N] = static_cast<double>(
          cnt +
          1); // Set to 1. to have the CPV initialized to all 1/cnt instead of 1
    } else {
...
void Genes::observedCodons(std::vector<std::vector<int> > &aObservedCodons,
  ...
  for (size_t j = 0; j < mSiteMultiplicity.size(); ++j) {
    ...
    if (s.size() >= 61 ||  // all the possible codons, no point to aggregate.
        ...
        (aAggregate == 2 &&
         maxMinCodonDistance(s) >
             2) // aggregation mode = 2 and huge edit distance
        ) {
      s.clear();
    } else {
      ++naggr;
  ...
  for (size_t j = 0; j < mSiteMultiplicity.size(); ++j) {
    ...
    for (int i = 0; i < N; i++) {
      int pos =
          std::find(aObservedCodons[j].begin(), aObservedCodons[j].end(), i) -
          aObservedCodons[j].begin();
      s.push_back(pos);
    }
    // std::cout << "pos=" << j << ",size=" << s.size() << std::endl;
        ...
        if (it2->second.size() == 1) {
          ...
          for (size_t i = 0; i < 3; i++)
            if (it1->first[i] != it2->first[i])
              ++dist;
          mMapCodonPairToDistance.insert(std::make_pair(
              std::make_pair(it1->second[0], it2->second[0]), dist));
        }
}
...
size_t Genes::maxMinCodonDistance(std::vector<int> aCodons) const {
  ...
  for (std::vector<int>::iterator it1 = aCodons.begin(); it1 != aCodons.end();
       it1++) {
    int minDist = -1; // minimal edit distance between this codon and some other
    for (std::vector<int>::iterator it2 = aCodons.begin(); it2 != aCodons.end();
         it2++)
      if (*it1 != *it2) {
        std::map<std::pair<int, int>, size_t>::const_iterator dist =
            mMapCodonPairToDistance.find(std::make_pair(*it1, *it2));
        if (dist == mMapCodonPairToDistance.end())
          FastCodeMLFatal("Couldn't find codon distance.");
        if (minDist < 0 || minDist > dist->second)
          minDist = dist->second;
      }
