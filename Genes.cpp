...
void Genes::setLeaveProb(double* aLeaveProbVect) const {
	...
	if (cnt == 0) {
		...
		for (size_t i = 0; i < cnt; ++i)
			aLeaveProbVect[i] = 1.; // Set to 1./cnt to have the CPV initialized to all 1/cnt instead of 1
#endif
	}
}
	...
	for (; is1 != end1; ++is1) {
		...
		if (!found)
			throw FastCodeMLFatal(
					"Mismatch between species in tree and genes.");
	}
}
...
void Genes::readFile(const char* aFilename, bool aCleanData) {
	...
	for (j = 0; j < ncodons; ++j) {
		for (i = 0; i < nspecies; ++i) {
			...
			if (len == 0) {
				std::ostringstream o;
				o << "Invalid codon at site " << j + 1 << " for specie "
						<< i + 1;
				throw FastCodeMLFatal(o);
			}
	...
	size_t valid_codons = std::count(codon_multiplicity.begin(),
			codon_multiplicity.end(), 1);
	if (valid_codons == 0)
	...
	if (mVerboseLevel >= VERBOSE_INFO_OUTPUT) {
		...
		if (num_ambiguous)
			std::cout << "Ambiguous:	" << std::setw(6) << num_ambiguous << "/"
					<< ncodons << std::endl;
		if (num_gaps)
			std::cout << "Gaps removed: " << std::setw(6) << num_gaps
					<< std::endl;
	}
	...
	if (mVerboseLevel >= VERBOSE_INFO_OUTPUT) {
		std::cout << "Sites:		" << std::setw(6) << mSiteMultiplicity.size()
				<< "/" << ncodons << std::endl;
		int multi_codons = static_cast<int>(std::count_if(
				codon_multiplicity.begin(), codon_multiplicity.end(),
	...
	for (i = 0; i < 4; ++i) {
		for (j = 0; j < 4; ++j) {
			for (k = 0; k < 4; ++k) {
				...
				codons.insert(
						std::pair<std::string, int>(std::string(codon), idx));
			}
		}
	}
