...
protected:
	...
	ProbabilityMatrixSet(size_t aNumMatrices, unsigned int aNumSets,
			...
			mMatrixSpace(NULL), mMatrices(NULL), mNumMatrices(
					static_cast<int>(aNumMatrices)), mNumSets(aNumSets), mFgBranch(
					0) {
		mMatrixSpace = static_cast<double*>(alignedMalloc(
				sizeof(double) * aNumMatSets * aNumMatrices * MATRIX_SLOT,
				CACHE_LINE_ALIGN));
		if (!mMatrixSpace)
...
#else
		for (int r = 0; r < N; ++r) {
			aGout[r] = mMatrices[aSetIdx * mNumMatrices + aBranch][r * N
					+ aCodon];
		}
#endif
	}
...
public:
	...
	///
	void fillMatrixSet(const TransitionMatrix& aQw0,
			const TransitionMatrix& aQ1, const TransitionMatrix& aQw2,
			double aSbg, double aSfg, const std::vector<double>& aParams);
...
/*                              equivalent classes supporting multiple foreground branches                           */
...
protected:
	...
	mfgProbabilityMatrixSet(size_t aNumMatrices, unsigned int aNumSets,
			unsigned int aNumMatSets) :
			mMatrixSpace(NULL), mMatrices(NULL), mNumMatrices(
					static_cast<int>(aNumMatrices)), mNumSets(aNumSets) {
		mMatrixSpace = static_cast<double*>(alignedMalloc(
				sizeof(double) * aNumMatSets * aNumMatrices * MATRIX_SLOT,
				CACHE_LINE_ALIGN));
		if (!mMatrixSpace)
