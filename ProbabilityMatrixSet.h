...
#ifdef USE_AGGREGATION
  void doTransitionAG(unsigned int aSetIdx, unsigned int aBranch,
                      const double *aGin, double *aGout,
                      const std::vector<int> &aObservedCodons,
                      const std::vector<int> &aMapObservedToState) const {
    int nObserved = aObservedCodons.size();
    /*
                    std::cout << "gin=";
                    for (std::vector<int>::const_iterator
       it=aObservedCodons.begin(); it !=aObservedCodons.end(); ++it)
                                    std::cout << aGin[*it] << " ";
                    std::cout << aGin[N+1] << "\n";
                    */
      ...
      if (aMapObservedToState[r] < nObserved) {
        ...
        for (int c = 0; c < N; ++c)
          if (aMapObservedToState[c] < nObserved)
            ...
            x += mMatrices[aSetIdx * mNumMatrices + aBranch][r * N + c] *
                 aGin[c];
          else {
            rToUnobserved +=
                mMatrices[aSetIdx * mNumMatrices + aBranch][r * N + c];
            unobservedToR +=
                mCodonFreq[c] *
                mMatrices[aSetIdx * mNumMatrices + aBranch][c * N + r];
          }
...
#ifdef USE_AGGREGATION
  void doTransitionAtLeafAG(unsigned int aSetIdx, unsigned int aBranch,
                            int aCodon, double *aGout,
                            const std::vector<int> &aObservedCodons,
                            const std::vector<int> &aMapObservedToState) const {
    int nObserved = aObservedCodons.size();
    ...
    std::cout << "matrix_col=";
    for (std::vector<int>::const_iterator it=aObservedCodons.begin(); it
    !=aObservedCodons.end(); ++it)
                    std::cout <<
    mMatrices[aSetIdx*mNumMatrices+aBranch][*it*N+aCodon] << " ";
    std::cout << mMatrices[aSetIdx*mNumMatrices+aBranch][(N+1)*N+aCodon] <<
    "\n"; */

    //
    // aCodon is always in observed set. So we calculate only observed ->
    // observed and unobserved -> observed
    ...
    for (int r = 0; r < N; ++r) {
      ...
      if (aMapObservedToState[r] == nObserved) {
        ...
        x += mCodonFreq[r] *
             mMatrices[aSetIdx * mNumMatrices + aBranch][r * N + aCodon];
      }
    }
