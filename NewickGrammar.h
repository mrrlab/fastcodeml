...
struct NewickGrammar : public grammar<NewickGrammar> {
  ...
  template <typename ScannerT> struct definition {
    definition(NewickGrammar const & /*self*/) {
      tree = nodelist >> !full_label >> !colon_plus_len >>
             no_node_d[*space_p >> ch_p(';')];

      nodelist = no_node_d[ch_p('(')] >>
                 subtree % (no_node_d[ch_p(',') >> *space_p]) >>
                 no_node_d[ch_p(')')];

      subtree =
          nodelist >> !full_label >> !colon_plus_len >> no_node_d[*space_p] |
          full_label >> !colon_plus_len >> no_node_d[*space_p];

      colon_plus_len = no_node_d[ch_p(':') >> *space_p] >> branch_length;

      branch_length = real_p;

      full_label = no_node_d[ch_p('#')] >> marker |
                   label >> !(no_node_d[ch_p('#')] >> marker);
