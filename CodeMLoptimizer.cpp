...
inline static void xtoy(const double x[], double y[], int n) {
...
static inline double distance(const double *RESTRICT x,
...
double Ming2::minimizeFunction(std::vector<double> &aVars) {
  ...
  // if(sts < 0 && mVerbose > 0) std::cout << "Check ming2 convergence" <<
  // std::endl;
...
int Ming2::ming2(FILE *fout, double *f, double x[], const double xl[],
  ...
  for (iround = 0; iround < mMaxIterations; ++iround) {
    // Check if the optimization can be stopped in advance due to LRT non
    // satisfied
    if (mStopIfBigger && *f < mThreshold)
      throw FastCodeMLEarlyStopLRT();

    if (fout) {
      fprintf(fout, "\n%3d %7.4f %13.6f  x: ", iround, sizep0, f0);
      for (j = 0; j < n; ++j)
        fprintf(fout, "%8.6f  ", x0[j]);
      fprintf(fout, "\n");
      fflush(fout);
    }
    ...
    h = min2(h, am / 5.0);
    *f = f0;
    double alpha = LineSearch2(f, x0, p, h, am, min2(1e-3, rel_error), tv,
                               iround, n); /* n or nfree? */
    ...
    if (alpha <= 0) {
      ...
      if ((n == 1 || goodtimes >= Ngoodtimes) &&
          sizep < (rel_error > 1e-5 ? 1 : .001) &&
          H_end(x0, x, f0, *f, rel_error, rel_error, n)) {
        break;
      }
    }
    ...
    if (w > 10 * sizep / nfree) /* *** */
    {
      ...
      for (i1 = 0; i1 < nfree; ++i1)
        memcpy(&H[i1 * (nfree + 1)], &C[i1 * nfree], nfree * sizeof(double));
...
bool Ming2::H_end(const double x0[], const double x1[], double f0, double f1,
...
double Ming2::fun_LineSearch(double t, const double x0[], const double p[],
                             double x[], int n) {
  for (int i = 0; i < n; ++i)
    x[i] = x0[i] + t * p[i];
  // return ((*fun) (x, n));
...
double Ming2::LineSearch2(double *f, const double x0[], const double p[],
  ...
  for (ii = 0; ii < maxround; ii++) {
    ...
    if (mNoisy > 2 &&
        ((a4 < a2 && f4 > f1) || (a4 > a2 && f4 > f3))) // CMV added correct #if
    {
...
void Ming2::gradientB(int n, const double x[], double f0, double g[],
  ...
  for (i = 0; i < n; ++i) {
    ...
    if (xmark[i] == 0 && (mAlwaysCenter || sizep < 1)) /* central */
    {
      ...
      g[i] = (-mModel->computeLikelihood(x1, n, mTraceFun) +
              mModel->computeLikelihood(x0, n, mTraceFun)) /
             (eh * 2.0);
