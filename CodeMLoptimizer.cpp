...
inline static void zero(double x[], int n) {
	...
	for (iround = 0; iround < mMaxIterations; ++iround) {
		...
		*f = f0;
		...
		for (i = 0; i < n; i++) /* add constraints, reduce H */
		{
			...
			for (i1 = 0, nfree--; i1 < nfree; i1++)
				for (i2 = 0; i2 < nfree; ++i2)
					H[i1 * nfree + i2] = C[(i1 + (i1 >= it)) * (nfree + 1) + i2
							+ (i2 >= it)];
		}
		...
		if (w > 10 * sizep / nfree) /* *** */
		{
			...
			for (i1 = 0; i1 < nfree; ++i1)
				memcpy(&H[i1 * (nfree + 1)], &C[i1 * nfree],
						nfree * sizeof(double));
		...
		for (i = 0; i < nfree; ++i)
			for (j = 0; j < nfree; ++j)
				H[i * nfree + j] += ((1 + w / v) * s[i] * s[j] - z[i] * s[j]
						- s[i] * z[j]) / v;
	...
	for (i = 0; i < n; ++i) {
		...
		if (xmark[i] == 0 && (mAlwaysCenter || sizep < 1)) /* central */
		{
			...
			g[i] = (-mModel->computeLikelihood(x1, n, mTraceFun)
					+ mModel->computeLikelihood(x0, n, mTraceFun)) / (eh * 2.0);
