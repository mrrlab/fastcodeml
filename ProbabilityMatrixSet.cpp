...
void mfgProbabilityMatrixSetH0::initializeSet(std::set<int> aFgBranchSet)
{
	...
	for(int branch=0; branch < num_matrices; ++branch)
	{
		...
		if (mFgBranchSet.find(branch)!=mFgBranchSet.end())
			mMatrices[branch+num_matrices*2] = &mMatrixSpace[(num_matrices+branch)*MATRIX_SLOT];
...
void mfgProbabilityMatrixSetH1::initializeSet(std::set<int> aFgBranchSet)
{
	...
	for(int branch=0; branch < num_matrices; ++branch)
	{
		...
		{
			mMatrices[branch+num_matrices*2] = mMatrices[branch+num_matrices*3] = &mMatrixSpace[(2*num_matrices+branch)*MATRIX_SLOT];
			//mMatrices[mFgBranch+num_matrices*2] = mMatrices[mFgBranch+num_matrices*3] = &mMatrixSpace[(2*num_matrices+mFgBranch)*MATRIX_SLOT];
		}
	}
}
...
#endif
		{
		const double t = (mFgBranchSet.find(branch)!=mFgBranchSet.end()) ? params[branch]/aSfg : params[branch]/aSbg;
...
void mfgProbabilityMatrixSetH0::setMatrices(size_t aBranch, const double** aMatricesPtr)
{
	memcpy(&mMatrixSpace[0*mNumMatrices*MATRIX_SLOT+aBranch*MATRIX_SLOT], aMatricesPtr[0], N*N*sizeof(double));
	memcpy(&mMatrixSpace[1*mNumMatrices*MATRIX_SLOT+aBranch*MATRIX_SLOT], aMatricesPtr[1], N*N*sizeof(double));
}
...
#pragma omp single
	{
		...
		#pragma omp task untied
#endif
		for (std::set<int>::iterator it=mFgBranchSet.begin(); it!=mFgBranchSet.end(); ++it)
		aQw2.computeFullTransitionMatrix(&mMatrixSpace[2*num_matrices*MATRIX_SLOT+(*it)*MATRIX_SLOT], params[*it]/aSfg);
	}
	}
}
...
void mfgProbabilityMatrixSetH1::restoreSavedMatrix(size_t aBranch)
{
	...
	{
		memcpy(&mMatrixSpace[(2*mNumMatrices+aBranch)*MATRIX_SLOT], mSaveQw2, N*N*sizeof(double));
	}
}
...
void mfgProbabilityMatrixSetH1::saveMatrix(size_t aBranch)
{
	...
	if(mFgBranchSet.find(static_cast<int>(aBranch)) != mFgBranchSet.end())
	{
		memcpy(mSaveQw2, &mMatrixSpace[(2*mNumMatrices+aBranch)*MATRIX_SLOT], N*N*sizeof(double));
	}
}
...
void mfgProbabilityMatrixSetH1::setMatrices(size_t aBranch, const double** aMatricesPtr)
{
	memcpy(&mMatrixSpace[0*mNumMatrices*MATRIX_SLOT+aBranch*MATRIX_SLOT], aMatricesPtr[0], N*N*sizeof(double));
	memcpy(&mMatrixSpace[1*mNumMatrices*MATRIX_SLOT+aBranch*MATRIX_SLOT], aMatricesPtr[1], N*N*sizeof(double));
	if(mFgBranchSet.find(static_cast<int>(aBranch)) != mFgBranchSet.end())
	{
		memcpy(&mMatrixSpace[(2*mNumMatrices+aBranch)*MATRIX_SLOT], aMatricesPtr[2], N*N*sizeof(double));
	}
}


void mfgProbabilityMatrixSetBEB::fillMatrixSet(const TransitionMatrix& aQfg, const TransitionMatrix& aQbg, double aSbg, double aSfg, const std::vector<double>& aParams)
{
	const int num_matrices = mNumMatrices;
	const double* params = &aParams[0];
