	...
	try {
		if (mNumGoodFreq == N) {
			...
			for (i = 0; i < N; ++i) {
				...
				for (j = 0; j < i; ++j) {
					mU[i * N + j] = mU[j * N + i] = mQ[i * N + j]
							* mSqrtCodonFreq[i] / mSqrtCodonFreq[j];
				}
			}
			...
			for (i = 0, inew = 0; i < N; ++i) {
				if (mGoodFreq[i]) {
					for (j = 0, jnew = 0; j < i; ++j) {
						if (mGoodFreq[j]) {
							mU[inew * mNumGoodFreq + jnew] = mU[jnew
									* mNumGoodFreq + inew] = mQ[i * N + j]
									* mSqrtCodonFreq[i] / mSqrtCodonFreq[j];
							++jnew;
						}
					}
			...
			for (i = N - 1, inew = mNumGoodFreq - 1; i >= 0; --i) {
				if (mGoodFreq[i]) {
					for (j = N - 1, jnew = mNumGoodFreq - 1; j >= 0; --j)
						if (mGoodFreq[j]) {
							mV[i * N + j] = mU[jnew * mNumGoodFreq + inew]
									* mSqrtCodonFreq[j];
							--jnew;
						} else {
							mV[i * N + j] = (i == j) ? 1. : 0.;
			...
			for (i = N - 1, inew = mNumGoodFreq - 1; i >= 0; --i) {
				if (mGoodFreq[i]) {
					for (j = N - 1, jnew = mNumGoodFreq - 1; j >= 0; --j)
						if (mGoodFreq[j]) {
							mU[i * N + j] = mU[inew * mNumGoodFreq + jnew]
									/ mSqrtCodonFreq[i];
							--jnew;
						} else {
							mU[i * N + j] = (i == j) ? 1. : 0.;
