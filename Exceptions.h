...
public:
  ...
  /// Exception that prints a message (previously built into a
  /// std::ostringstream) before termination.
  ///
  /// @param[in] aMessage The message to be printed before termination (has been
  /// formatted by printing into a std::ostringstream).
  ///
  FastCodeMLFatal(const std::ostringstream &aMessage)
      : runtime_error(aMessage.str().c_str()) {}
};

/// Early successful termination exception.
/// It does not signal a fatal error, but something like having printed the
/// help.
///
...
/// It does not signal a fatal error, but that the optimization stopped because
/// LRT cannot be satisfied.
///
...
public:
  ...
  /// @param[in] aMessage The message to be printed before termination
  ///
  FastCodeMLMemoryError(const char *aMessage)
      : bad_alloc(), mMessage(aMessage) {}
